...
from points_int import RATIO, MIN_X, MAX_X, MIN_Y, MAX_Y
...
def map(days, geohashes, prefix=None, open_file=True):
    ...
    for d, day in enumerate(days):
        for period, location in enumerate(day):
            color = location / 100, 1., 1., 1.     
            geohash = geohashes[location]
            lonlat = geo.geohash_decode(geohash)
